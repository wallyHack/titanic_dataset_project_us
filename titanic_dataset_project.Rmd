...
# 1.- The data domain
we will perform an analysis on the titanic dataset to identify 
...
Survived
: Indicates if the passenger survived. (0 = No, 1 = Yes).

Pclass
: Define the socioeconomic class of the passenger. (1 = Lower class, 2 = Middle class, 3 = Upper class).

Name
: Name of the passenger.

Sex
: Gender of the passenger. (Male and/or Female).
...
- we show the first 10 records of the dataset.
```{r, include=TRUE}
head(titanic, 10)
``` 

\newpage
- we show the structure of the data and/or the data types of the attributes.\
```{r, include=TRUE}
str(titanic)
```
...
We observed that the majority of the passengers were young people because they were between 20 and 30 years old. And it is a **non-symmetric distribution** since it is skewed to the right, because the mean age is greater than the median.
```{r, include=TRUE}
hist(titanic$Age, main = "Titanic Passenger Age Histogram", xlab = "Age", ylab = "Passengers")
```

\newpage
We observed that the majority of passengers paid less than $ 100 on their boarding tickets.
And it is a **non-symmetric distribution** since it is skewed to the right because the average rate is higher than the average rate.
```{r, include=TRUE}
hist(titanic$Fare, main = "Titanic Passenger Fare Histogram", xlab = "Fare", ylab = "Passengers")
```

\newpage
We note that the majority of the passengers traveling on the titanic died and less than half survived. Showing the figures 549 passengers died and 342 survived.
```{r, include=TRUE}
hist(titanic$Survived, main = "Titanic Passenger Survival Histogram", xlab = "Survival", ylab = "Passengers")
```

\newpage
We observed that the majority of the passengers were wealthy people or with money since they belong to the upper class and less than half of the passengers were distributed between the middle and lower class.
```{r, include=TRUE}
...
We observe that the majority of passengers are in **the age range of 20 to 38 years.**
...
We observed that the majority of passengers paid a fare and / or ticket cost of around ** 8 to 31 dollars. ** And this makes the high costs considered outliers or anomalies.
```{r, include=TRUE}
...
We observe that there are many passengers between the ages of 20 and 40 who bought a ticket for less than $ 100.
And there are very few passengers who bought a ticket with a cost greater than 100 dollars.
```{r, include=TRUE}
...
\newpage
We note that there is no relationship between the cost of the ticket and the chances of survival.
```{r}
